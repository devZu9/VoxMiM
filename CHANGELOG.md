...
- **Поле для триггер-фраз в настройках** — многострочное `text_area`, одна строка = одна фраза.
- **Настройка окна детекции (wake_window_sec)** — в Fenestra, на вкладке «Запись».
- **Настройка макс. длительности записи (wake_max_duration)** — в Fenestra.
