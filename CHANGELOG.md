...
- **Сохранение позиции окна настроек** — `window_x`/`window_y` пишутся в `config.json` при каждом `apply()`. При открытии окно восстанавливается на сохранённой позиции (EnumWindows + PID).
- **Сохранение текущей вкладки настроек** — `cur_tab` сохраняется в `config.json` при каждом переключении вкладки (SetTab → apply()).
- **Поля `window_x`, `window_y`, `cur_tab` в `Config` основного приложения** — `Config::save()` теперь знает про эти поля и не теряет их при записи.
...
- **Окно настроек (voxmim-settings):**
  ...
  - `apply()` теперь `&mut self` — перед сохранением обновляет позицию окна через `GetActiveWindow()`
