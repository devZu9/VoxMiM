...
- **Дефолтный таймаут whisper: 120 → 60 секунд** — быстрее реакция на сбой сервера.
- **Ретраи с таймаутом 30 секунд** — если первая попытка не удалась, ретраи используют 30s вместо полного таймаута. После ретрая исходный таймаут восстанавливается.

### Исправлено

- **State = Idle сразу после первой ошибки whisper** — раньше `RecordingResult` отправлялся только после исчерпания всех 3 ретраев. Main thread оставался в `Processing` до 7 минут. Теперь первая же ошибка отправляет пустой `RecordingResult`, хоткей работает немедленно. Ретраи продолжаются в том же потоке — если повезёт, результат вставится постфактум.
