...
- **VAD: агрессивность (0–3) → прямой порог RMS (f32)** — вместо выбора из 4 уровней теперь задаётся числовое значение порога с шагом 0.002. Дефолт: 0.008. Поле ввода + спин-кнопки (▲/▼) + кнопка сброса на дефолт (↩). Название: «Чувствительность микрофона».
- **VadConfig: `aggressiveness: u32` → `threshold: f32`** — изменение схемы конфига. Миграция старых конфигов: `aggressiveness` → `threshold` при загрузке.
- **reload_config() — синхронизация трея** — при перезагрузке конфига из окна настроек теперь обновляются галочки в трее (VAD, Wake Word).
- **Startup: server-режим без проверки whisper-cli** — если оба движка в server-режиме, `ensure_whisper_bins` пропускается, лишний лог не выводится.
- **`"Engine: модель"` → `debug!`** — лог загрузки модели не шумит при `RUST_LOG=info`.

### Добавлено

- **Синхронизация окна настроек ↔ трей** — фоновый поток `config-watch` в окне настроек проверяет `mtime` конфига каждые 500мс. Изменение из трея сразу отображается в UI окна настроек и наоборот.
- **Кнопка возврата (↩) в чувствительности микрофона** — сбрасывает порог на 0.008.

### Исправлено

- **Миграция `aggressiveness` перезаписывала пользовательский `threshold`** — миграция бежала при каждом запуске, конвертируя `aggressiveness: 1` → `threshold: 0.03`, перетирая выставленный пользователем 0.008. Исправлено: миграция срабатывает только если `threshold` отсутствует в JSON. Старое поле `aggressiveness` удаляется при сохранении из окна настроек.
- **VAD-чувствительность** — RMS=0.01 (нормальная речь на некоторых микрофонах) теперь корректно детектируется как речь при пороге 0.008.

### Версии

- **voxmim**: 0.8.2 → 0.9.0
- **voxmim-settings**: 1.2.0 → 1.3.0
