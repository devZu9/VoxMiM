...
- **Pending WAV — фраза не теряется при ошибке сервера.** Если whisper-server не ответил, сэмплы сохраняются в `wavs/pending.wav`. При следующем цикле whisper worker сначала пробует распознать `pending.wav`. Если сервер восстановился — фраза вставляется как обычно.
- **Индикатор перезапуска в трее.** При ошибке сервера иконка мигает hourglass (как при старте), чтобы было видно: идёт восстановление. После успешного ретрая — возвращается к IDLE.

### Изменено

- **Ретрай удалён.** Вместо 3 попыток в whisper thread — одна попытка и сохранение в pending.wav. Whisper worker не блокирует pipeline.

### Исправлено

- **Зависание после ретрая.** Раньше retry в том же потоке блокировал новые записи. Теперь thread сразу переходит к recv(), а pending обрабатывается на следующем цикле.
