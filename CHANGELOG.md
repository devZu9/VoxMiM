...
- **`stop_server()` — без `child.wait()`.** Заменён на `taskkill /f /im whisper-server.exe` (fire-and-forget). Никаких блокировок на `kill()`/`wait()`, которые могли висеть, если процесс был в нестабильном состоянии после таймаута.
- **`http_post()` — добавлен `set_write_timeout()`.** Раньше был только `read_timeout`. Если сервер не читал тело запроса, `write_all(body)` мог висеть бесконечно.
- **`http_get()` — добавлен `set_write_timeout(5s)`.** Для консистентности.
- **`kill()` метод удалён.** В `ensure_server` ошибки обрабатываются через прямой `child.kill()` + `child.wait()` (гарантированно быстрый — процесс только что запущен и не висел).

### Исправлено

- **Зависание после таймаута whisper.** Раньше `stop_server()` вызывал `child.wait()` на зависшем процессе — программа не отвечала сколь угодно долго. Теперь `taskkill` убивает процесс гарантированно, без ожидания подтверждения.
