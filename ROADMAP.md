...
> **v0.7.3: Реализован и протестирован. Pre-speech таймаут (start_timeout_secs), silence timeout, tap-режим, принудительная остановка.**
...
## Фаза 11: Триггер-фраза (Wake Word) ⚠️

> **Код написан, но не тестирован — функционал не подтверждён.**

- [x] Единый аудио-захват (fan-out через `start_capture_multi`)
- [x] Детекция wake word через whisper-cli small-модель
- [x] Накопление команды после триггера + транскрибация
- [x] Переключение через трей-меню «Голосовая активация»
- [x] Работает без конфликта с VAD (разные потоки, общий аудио-стрим)

---

## Фаза 11a: Wake Word + VAD Integration ⚠️

> **Код написан, но не тестирован — функционал не подтверждён.**

- [x] VAD только автостоп (не автостарт)
- [x] Режимы: PTT / VAD / Wake / Wake+VAD
- [x] Tap-режим: при VAD Insert переключает запись
- [x] Принудительная остановка: повторный Insert при VAD/wake
- [x] Единый аудио-пайплайн без WASAPI-конфликтов
...
| **VAD** | +4 | VAD-режим: автостоп по тишине + pre-speech таймаут | 🟢 **v0.7.3** |
...
| **WakeWord** | +11 | Триггер-фраза вместо горячей клавиши | ⚠️ **код есть, не тестирован** |
| **Wake+VAD** | +11a | Wake + VAD без конфликтов | ⚠️ **код есть, не тестирован** |
