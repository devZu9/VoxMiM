# VoxMiM v0.9.2 — Итоговая сводка

## Что сделано

**VoxMiM** — голосовой ввод текста на Rust. По мотивам VoxBee с исправлением ключевой ошибки (разорванные слова).

> **v0.9.2:** Таймаут распознавания whisper (настраивается 10–180 сек). Retry-цикл 3 попытки при сбое сервера с backoff. Пустой `RecordingResult` при ошибке — state не застревает в `Processing`. Логи `warn!`/`error!` на каждый сбой. UI-настройка на вкладке «Постобработка».
>
...
- **voxmim**: 0.9.2 (таймаут whisper + retry, фикс зависания на сервере)
