# VoxMiM v0.7.3 — Итоговая сводка

## Что сделано

**VoxMiM** — голосовой ввод текста на Rust. По мотивам VoxBee с исправлением ключевой ошибки (разорванные слова).

> **v0.7.3:** VAD (автостоп) доведён до рабочего состояния. Добавлен pre-speech таймаут (2 сек ожидания речи после Tap Insert). Исправлен автоповтор Insert (трель Start/Stop). Сброс HOOK_REC после автостопа. Очистка audio_buf при старте записи. Версионность окна настроек (voxmim-settings 1.0.3).
...
| VAD (Автостоп) | ✅ | Energy-based, pre-speech таймаут, silence timeout, tap-режим |
...
- [x] **VAD (Автостоп)** — v0.7.3, pre-speech таймаут, протестирован, работает
...
- [x] **Независимые версии voxmim (0.x.x) и voxmim-settings (1.0.3)** — разные семвер-линии
